...
console.log('--- Flatohh Deep CJS Compatibility Check ---');
console.log(`Node Version: ${process.version}`);
console.log(`Platform: ${process.platform} (${process.arch})`);

try {
  // Point to the CJS build explicitly
  const libPath = path.resolve(__dirname, '../dist/index.cjs');
  console.log(`\nLoading library from: ${libPath}`);
  ...
  console.log('Library loaded successfully. Starting deep checks...\n');

  // 1. Test Deep Flattening & Unflattening
  ...
  console.log('\nStatus: PASSED');
  console.log(`The CJS build is fully operational on Node ${process.version}`);
  process.exit(0);
